...
export default function GiveExam() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`${import.meta.env.VITE_API_URL}/api/exams/${examId}/progress`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        if (res.ok) {
          const prog = await res.json();
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`${import.meta.env.VITE_API_URL}/api/exams/${examId}/student`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        if (!res.ok) throw new Error();
        const data = await res.json();
  ...
  useEffect(() => {
    if (!submitted && exam && timeLeft != null) {
      (async () => {
        try {
          await fetch(`${import.meta.env.VITE_API_URL}/api/exams/${examId}/progress`, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              Authorization: `Bearer ${token}`
  ...
  async function handleSubmit() {
    ...
    try {
      await fetch(`${import.meta.env.VITE_API_URL}/api/exams/${examId}/submit`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`
